...
def simulate_multi_stage_dungeon(state: GameState, party: List[Hero], dungeon: Dungeon) -> List[str]:
    ...
    _apply_expedition_xp_and_cleanup(state, party, xp_earned, expedition_summary)

    return expedition_summary


def simulate_auto_dungeon(state: GameState, party: List[Hero], dungeon: Dungeon) -> List[str]:
    for hero in party:
        hero.reset_health_for_expedition()

    expedition_summary = [
        bold("=== Dungeon Expedition Summary ==="),
        f"Dungeon: {dungeon.name}",
        info("The guild captain automatically chose each room path."),
    ]
    ...
    for room_number in range(1, dungeon.room_count + 1):
        ...
        room_option = choose_auto_room_option(dungeon, room_number, party)
...
def choose_auto_room_option(dungeon: Dungeon, room_number: int, party: List[Hero]):
    ...
    return max(options, key=lambda option: priority.get(option.room_type, 0))


def _apply_expedition_xp_and_cleanup(
    state: GameState,
    party: List[Hero],
    xp_earned: int,
    expedition_summary: List[str],
) -> None:
