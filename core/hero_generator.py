"""
core/hero_generator.py

Generates hero instances for the contract market, applying class rules,
career phase logic, progression steps, and contract valuation.

Attitude names updated to v11: Humble / Pragmatic / Ambitious / Honorable / Opportunistic.
"""
...
def choose_class_for_state(rules: dict, state) -> str:
    unlocked = getattr(state.guild_upgrades, "unlocked_classes", ["Warrior"])
    valid = [c for c in unlocked if c in rules.get("classes", {})]
    return random.choice(valid) if valid else "Warrior"


def scale_stats_for_level(stats: dict, level: int, hero_class: str) -> dict:
    if level <= 1:
        return dict(stats)
    primary = {"Warrior": ["might"], "Rogue": ["agility"], "Cleric": ["spirit"], "Mage": ["mind"]}.get(hero_class, [])
    scaled = dict(stats)
    for _ in range(level - 1):
        for s in primary:
            scaled[s] = int(scaled.get(s, 0)) + 1
        r = random.choice(["might", "agility", "mind", "spirit"])
        scaled[r] = int(scaled.get(r, 0)) + 1
    return scaled


# ---------------------------------------------------------------------------
# Negotiation profiles — v11 attitude names
# ---------------------------------------------------------------------------

_ATTITUDE_BASE_PROFILES = {
    "Humble": {
        "money_per_campaign_weight": 0.68,
        "term_weight":               0.12,
        "mentorship_weight":         0.10,
        "safety_weight":             0.04,
    ...
    "Ambitious": {
        ...
        "safety_weight":             0.02,
        "development_weight":        0.09,
    },
    "Honorable": {
        "money_per_campaign_weight": 0.62,
        "term_weight":               0.08,
        "mentorship_weight":         0.06,
        "safety_weight":             0.06,
        "development_weight":        0.04,
    },
    "Opportunistic": {
        "money_per_campaign_weight": 0.84,
        "term_weight":               0.06,
...
_DEFAULT_PROFILE = _ATTITUDE_BASE_PROFILES["Pragmatic"]


def assign_negotiation_profile(hero: Hero, developmental: bool = False) -> Hero:
    profile = dict(_ATTITUDE_BASE_PROFILES.get(hero.contract_attitude, _DEFAULT_PROFILE))
    ...
    if phase == "Rookie":
        ...
        profile["money_per_campaign_weight"] = max(0.60, profile["money_per_campaign_weight"] - 0.05)
    elif phase == "Rising":
        preferred_campaigns = max(3, min(preferred_campaigns, 5))
    ...
    if developmental:
        ...
        profile["money_per_campaign_weight"] = max(0.58, profile["money_per_campaign_weight"] - 0.06)
...
def market_tier_for_hero(hero: Hero, developmental: bool = False) -> str:
    ...
    growth_rank = {"Mundane": 0, "Talented": 1, "Gifted": 2, "Heroic": 3, "Legendary": 4, "Mythic": 5}.get(hero.growth_rate, 1)
...
def _apply_training_steps(hero, steps: int) -> None:
    ...
    for _ in range(steps):
        candidates = [
            p for p in path_names
            if int(hero.training_path_progress.get(p, 0)) < len(CLASS_TRAINING_PATHS[hero.hero_class][p]["ranks"])
        ]
        if not candidates:
            break
        weights = []
        ...
        for path_name in candidates:
            ...
            if hero.hero_class == "Warrior":
                if phase in ("Rising", "Prime") and low in ("vanguard", "warden"): weight += 2
                if phase in ("Veteran", "Elder") and low == "captain": weight += 2
            elif hero.hero_class == "Rogue":
                if phase in ("Rookie", "Rising") and low == "scout": weight += 2
                if phase in ("Prime", "Veteran") and low in ("fixer", "shadow"): weight += 2
            elif hero.hero_class == "Cleric":
                if phase in ("Rising", "Prime") and low in ("templar", "shepherd"): weight += 2
                if phase in ("Veteran", "Elder") and low == "oracle": weight += 2
            elif hero.hero_class == "Mage":
                if phase in ("Rising", "Prime") and low == "arcanist": weight += 2
                if phase in ("Veteran", "Elder") and low in ("seer", "spellblade"): weight += 2
            if int(hero.training_path_progress.get(path_name, 0)) == 0: weight += 1
            weights.append(weight)
        chosen = random.choices(candidates, weights=weights, k=1)[0]
        spend_training_point(hero, chosen)


def _prior_development_steps_for_hero(hero) -> int:
    phase = career_phase_name(hero)
    if phase == "Rookie":  return 0
    if phase == "Rising":  return random.randint(0, 1)
    if phase == "Prime":   return random.randint(1, 3)
    if phase == "Veteran": return random.randint(2, 4)
    if phase == "Elder":
        return random.randint(1, 3) if hero.hero_class == "Rogue" else random.randint(3, 5)
    return 0


def build_base_hero(*, existing_names, state, class_name, age, level, stats,
                    signing_bonus, wage_per_year, contract_years, growth_rate, contract_attitude) -> Hero:
    return Hero(
        name=generate_name(existing_names),
        hero_class=class_name,
        age=age, level=level, xp=0, stats=stats,
        signing_bonus=signing_bonus, wage_per_year=wage_per_year,
...
def generate_hero(existing_names: Set[str], state) -> Hero:
    ...
    stats = {s: random.randint(int(r[0]), int(r[1])) for s, r in class_rules["stat_ranges"].items()}
    ...
    signing_bonus, wage_per_year = calculate_contract_values(
        hero_class=class_name, age=age, level=level, stats=stats,
        contract_attitude=contract_attitude, reputation=state.reputation,
        pricing_rules=rules.get("pricing", {}), attitude_multiplier_func=attitude_multiplier,
    )
    ...
    hero = build_base_hero(existing_names=existing_names, state=state, class_name=class_name,
                           age=age, level=level, stats=stats, signing_bonus=signing_bonus,
                           wage_per_year=wage_per_year, contract_years=contract_years,
                           growth_rate=random_growth_rate(), contract_attitude=contract_attitude)
...
def generate_developmental_hero(existing_names: Set[str], state) -> Hero:
    ...
    age   = choose_age_for_phase(class_name, phase_name)
    level = min(1 if phase_name == "Rookie" else 2, getattr(state.guild_upgrades, "recruit_level_cap", 1))
    ...
    hero = build_base_hero(existing_names=existing_names, state=state, class_name=class_name,
                           age=age, level=level, stats=stats,
                           signing_bonus=max(25, int(signing_bonus * 0.55)),
                           wage_per_year=max(1, int(wage_per_year * 0.60)),
                           contract_years=random.randint(2, 4),
                           growth_rate=random.choices(["Mundane", "Talented", "Gifted"], weights=[25, 55, 20], k=1)[0],
                           contract_attitude=contract_attitude)

    ensure_progression_fields(hero)
    hero.training_points = 0; hero.training_path_progress = {}
    hero.unlocked_subclasses = []; hero.unlocked_abilities = []
    hero.primary_subclass = None; hero.subclass = None; hero.special_ability = None
...
def generate_contract_market(state, count=None, include_developmental=True, developmental_count=3) -> List[Hero]:
    rules = load_hero_generation_rules()
    market_size = count or int(rules.get("market_size", 8))
    existing_names = (
        {h.name for h in state.roster} | {h.name for h in state.available_contracts}
        | {h.name for h in state.retired_heroes} | {h.name for h in state.fallen_heroes}
    )
    generated: List[Hero] = []
    normal_target = max(0, market_size - developmental_count) if include_developmental else market_size
    while len(generated) < normal_target:
        h = generate_hero(existing_names, state); existing_names.add(h.name); generated.append(h)
    if include_developmental:
        while len(generated) < market_size:
            h = generate_developmental_hero(existing_names, state); existing_names.add(h.name); generated.append(h)
    tier_rank  = {"Elite": 0, "Premium": 1, "Standard": 2, "Developmental": 3}
    phase_rank = {"Prime": 0, "Rising": 1, "Veteran": 2, "Rookie": 3, "Elder": 4}
    generated.sort(key=lambda h: (
        tier_rank.get(getattr(h, "market_tier", "Standard"), 9),
        phase_rank.get(career_phase_name(h), 9),
        -int(getattr(h, "level", 1)),
        -len(getattr(h, "unlocked_subclasses", []) or []),
        int(getattr(h, "age", 18)),
    ))
    return generated
