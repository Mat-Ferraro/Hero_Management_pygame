"""
systems/campaign/task_scoring.py

Calculates mission success chances, evaluates stat rules, and resolves
outcome bands for campaign tasks.

Equipment integration (fix):
  hero_dispatch_stats() now calls equipment_effects.apply_equipment_to_dispatch_stats()
  after building the base stat dict from raw hero stats, class bonuses, and
  subclass bonuses.  This means equipped items — including active synergy
  bonuses — actually contribute to mission outcomes.

  The equipment call is guarded with a try/except so a missing import or
  unequipped hero never crashes task resolution.
"""
...
from systems.equipment.equipment_effects import apply_equipment_to_dispatch_stats
...
# ---------------------------------------------------------------------------
# Dispatch stat calculation
# ---------------------------------------------------------------------------

def hero_dispatch_stats(hero) -> Dict[str, int]:
    """
    Build the full dispatch stat profile for a single hero.

    Sources applied in order:
      1. Base stats derived from raw hero stats + career phase modifiers.
      2. Class bonus (fixed per hero_class).
      3. Subclass bonus (matched by substring against subclass name).
      4. Equipment bonuses — direct item stat_bonuses + active synergy bonuses
         from equipment_effects.apply_equipment_to_dispatch_stats().
    """
    ...
    # Apply equipment bonuses (items + active synergies).
    # Guarded so unequipped heroes or import issues never crash dispatch.
    try:
        stats = apply_equipment_to_dispatch_stats(stats, hero)
    except Exception:
        pass
...
def combined_party_dispatch_stats(heroes: List[Any]) -> Dict[str, int]:
    ...
    for hero in heroes:
        for stat_name, value in hero_dispatch_stats(hero).items():
...
def stat_rule_has_weight(rule: Dict[str, Any]) -> bool:
    ...
    if mode == "minimum": return int(rule.get("target", 0)) > 0
    if mode == "range":   return int(rule.get("max", 0)) > 0
    if mode == "maximum": return int(rule.get("target", 0)) > 0
    return False
...
def evaluate_stat_rule(value: int, rule: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if mode == "maximum":
        target = max(0, int(rule.get("target", 0)))
        if value <= target:  return {"score": 1.0, "mode": mode}
        if value <= 0:       return {"score": 1.0, "mode": mode}
        if target <= 0:      return {"score": 0.0, "mode": mode}
...
def format_stat_rule_short(rule: Dict[str, Any]) -> str:
    rule = normalize_rule(rule)
    mode = rule["mode"]
    if mode == "minimum": return f"Need {int(rule.get('target', 0))}+"
    if mode == "range":   return f"Best {int(rule.get('min', 0))}-{int(rule.get('max', 0))}"
    if mode == "maximum": return f"Keep ≤{int(rule.get('target', 0))}"
    return "Any"
...
def preferred_class_bonus(task, heroes: List[Any]) -> float:
    ...
    if not preferred:
        return 0.0
    matches = sum(1 for h in heroes if getattr(h, "hero_class", None) in preferred)
...
def party_size_bonus(task, heroes: List[Any]) -> float:
    ...
    if not heroes:
        return 0.0
    return min(1.0, len(heroes) / max_heroes) * 0.05
...
def outcome_band_from_fit_and_roll(
    success_chance: float, fit_score: float, roll_value: float
) -> str:
...
def injury_profile_for_band(outcome_band: str) -> Dict[str, float]:
    p = outcome_profile(outcome_band)
    return {
        "injury_chance":  float(p["injury_chance"]),
        "extra_rest_min": float(p["extra_rest_min"]),
        "extra_rest_max": float(p["extra_rest_max"]),
    }


def satisfaction_delta_for_band(outcome_band: str) -> int:
    return int(outcome_profile(outcome_band)["satisfaction_delta"])


def payout_multiplier_for_band(outcome_band: str) -> float:
    return float(outcome_profile(outcome_band)["payout_multiplier"])


def xp_multiplier_for_band(outcome_band: str) -> float:
    return float(outcome_profile(outcome_band)["xp_multiplier"])


# ---------------------------------------------------------------------------
# Success chance + resolution
# ---------------------------------------------------------------------------
...
def calculate_task_success_chance(task, heroes: List[Any]) -> Dict[str, Any]:
    ...
    fit_score = (
        sum(stat_breakdown[s]["score"] for s in active_stats) / len(active_stats)
        if active_stats else 1.0
    )
...
def resolve_task_outcome_from_chance(
    task, heroes: List[Any], roll_value: float
) -> Dict[str, Any]:
    ...
    base_injury        = injury_profile_for_band(outcome_band)

    injury_profile = {
        "injury_chance": _clamp(
            float(base_injury["injury_chance"]) * float(ability_data.get("injury_multiplier", 1.0)),
            0.0, 1.0,
        ),
        "extra_rest_min": float(base_injury["extra_rest_min"]),
        "extra_rest_max": float(base_injury["extra_rest_max"]),
    }

    satisfaction_delta = (
        satisfaction_delta_for_band(outcome_band)
        + int(ability_data.get("satisfaction_bonus", 0))
    )
